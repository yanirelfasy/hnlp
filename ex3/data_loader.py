...
class SentimentTreeBank(object):
    ...
    #our addition
    def get_rare_word_test_set(self):
        """
        :return: list of Sentence instances,with rare words, for the test part of the dataset
        """
        test_set = self.get_test_set()
        rare_words = get_rare_words_examples(test_set,self)
        self._rare_sent = []
        for i,val in enumerate(rare_words):
            self._rare_sent.append(test_set[val])
        return self._rare_sent

    #our addition
    def get_neg_word_test_set(self):
        """
        :return: list of Sentence instances,with negative words, for the test part of the dataset
        """
        test_set = self.get_test_set()
        neg_words = get_negated_polarity_examples(test_set)
        self._neg_sent = []
        for i,val in enumerate(neg_words):
            self._neg_sent.append(test_set[val])
        print(self._neg_sent)
        return self._neg_sent
...
if __name__ == '__main__':
    ...
    # get train set
    print(dataset.get_train_set()[:10])
    print(dataset.get_train_set()[3].sentiment_val)
    print(dataset.get_train_set()[3].sentiment_class)
    # get word counts dictionary
    print(dataset.get_word_counts())
